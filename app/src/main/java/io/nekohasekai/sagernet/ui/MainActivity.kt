...
class MainActivity : ThemedActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.fab.setOnClickListener {
            // when selected profile is the floating-window entry, the FAB should control the
            // floating window instead of starting VPN
            if (DataStore.serviceState.canStop) {
                SagerNet.stopService()
            } else {
                val selectedId = DataStore.selectedProxy
                val isFloatingProfile = try {
                    ProfileManager.getProfile(selectedId)?.displayName() == "悬浮窗"
                } catch (_: Exception) {
                    false
                }
        ...
        if (KIOSK_MODE && ::navigation.isInitialized) {
            navigation.menu.findItem(R.id.nav_group)?.isVisible = false
            navigation.menu.findItem(R.id.nav_route)?.isVisible = false
            navigation.menu.findItem(R.id.nav_settings)?.isVisible = false
            navigation.menu.findItem(R.id.nav_logcat)?.isVisible = false
            navigation.menu.findItem(R.id.nav_traffic)?.isVisible = false
            navigation.menu.findItem(R.id.nav_tools)?.isVisible = false
            navigation.menu.findItem(R.id.nav_tuiguang)?.isVisible = false
            navigation.menu.findItem(R.id.nav_faq)?.isVisible = false
            navigation.menu.findItem(R.id.nav_about)?.isVisible = false
        }
    ...
    fun displayFragmentWithId(@IdRes id: Int): Boolean {
        when (id) {
            ...
            R.id.nav_group -> if (!KIOSK_MODE) displayFragment(GroupFragment()) else return false
            R.id.nav_route -> if (!KIOSK_MODE) displayFragment(RouteFragment()) else return false
            R.id.nav_settings -> if (!KIOSK_MODE) displayFragment(SettingsFragment()) else return false
            R.id.nav_traffic -> if (!KIOSK_MODE) displayFragment(WebviewFragment()) else return false
            R.id.nav_tools -> if (!KIOSK_MODE) displayFragment(ToolsFragment()) else return false
            R.id.nav_logcat -> if (!KIOSK_MODE) displayFragment(LogcatFragment()) else return false
            R.id.nav_faq -> {
