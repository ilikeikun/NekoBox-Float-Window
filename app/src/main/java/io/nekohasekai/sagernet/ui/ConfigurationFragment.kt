...
import io.nekohasekai.sagernet.KIOSK_MODE
...
import io.nekohasekai.sagernet.plugin.PluginCleanup
import io.nekohasekai.sagernet.plugin.ConfigurationUiPluginConfig
import io.nekohasekai.sagernet.plugin.PluginManager
import io.nekohasekai.sagernet.ui.AppManagerActivity.Companion.EXTRA_READ_ONLY
import io.nekohasekai.sagernet.ui.FloatingWindowService
import io.nekohasekai.sagernet.ui.PluginManagerActivity
...
class ConfigurationFragment @JvmOverloads constructor(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ConfigurationUiPluginConfig.loadBackgroundDrawable(requireContext())?.let { drawable ->
            view.background = drawable
        }

        if (!select) {
            if (KIOSK_MODE) {
                toolbar.inflateMenu(R.menu.kiosk_menu)
            } else {
                toolbar.inflateMenu(R.menu.add_profile_menu)
            }
            toolbar.setOnMenuItemClickListener(this)
        } else {
            val safeTitleRes = if (titleRes != 0) titleRes else R.string.menu_configuration
            toolbar.title = getString(safeTitleRes)
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        if (item.itemId == R.id.action_plugin_manager) {
            startActivity(Intent(requireActivity(), PluginManagerActivity::class.java))
            return true
        }

        if (KIOSK_MODE) return true
        when (item.itemId) {
    ...
    class GroupFragment : Fragment() {
        ...
        fun setGroup(group: ProxyGroup, selectedItem: ProxyEntity? = null) {
            proxyGroup = group
        ...
        inner class ConfigurationAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>(),
            ProfileManager.Listener,
            GroupManager.Listener,
            ...
            private val VIRTUAL_PLUGIN_ID = -100L
            private val VIEW_TYPE_PROFILE = 0
            private val VIEW_TYPE_PLUGIN = 1
            ...
            override fun onCreateViewHolder(
                ...
                return if (viewType == VIEW_TYPE_PLUGIN) {
                    VirtualPluginHolder(
                        LayoutInflater.from(parent.context)
                            .inflate(R.layout.layout_virtual_plugin_profile, parent, false)
                    )
                } else {
                    ConfigurationHolder(
                        LayoutInflater.from(parent.context)
            ...
            override fun getItemViewType(position: Int): Int {
                return if (configurationIdList[position] == VIRTUAL_PLUGIN_ID) VIEW_TYPE_PLUGIN else VIEW_TYPE_PROFILE
            }
            ...
            override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
                if (holder is VirtualPluginHolder) {
                    holder.bind()
                } else if (holder is ConfigurationHolder) {
                    try {
            ...
            fun filter(name: String) {
                ...
                val hasPlugin = configurationIdList.contains(VIRTUAL_PLUGIN_ID)
                configurationIdList.clear()
                if (hasPlugin) configurationIdList.add(VIRTUAL_PLUGIN_ID)
            ...
            fun move(from: Int, to: Int) {
                if (configurationIdList[from] == VIRTUAL_PLUGIN_ID || configurationIdList[to] == VIRTUAL_PLUGIN_ID) return
            ...
            fun remove(pos: Int) {
                if (pos < 0 || configurationIdList.getOrNull(pos) == VIRTUAL_PLUGIN_ID) return
            ...
            override suspend fun onAdd(profile: ProxyEntity) {
                ...
                configurationListView.post {
                    ...
                    val hasPlugin = configurationIdList.contains(VIRTUAL_PLUGIN_ID)
                    val insertPos = if (hasPlugin) itemCount else itemCount
            ...
            override suspend fun onUpdated(data: TrafficData) {
                if (data.id == VIRTUAL_PLUGIN_ID) return
                try {
            ...
            override suspend fun onRemoved(groupId: Long, profileId: Long) {
                ...
                if (profileId == VIRTUAL_PLUGIN_ID) return
            ...
            fun reloadProfiles() {
                ...
                configurationListView.post {
                    configurationIdList.clear()
                    if (hasPlugin) {
                        configurationIdList.add(VIRTUAL_PLUGIN_ID)
                    }
                    configurationIdList.addAll(newProfileIds)
                    notifyDataSetChanged()

                    if (selectedProfileIndex != -1) {
                        val scrollIndex = if (hasPlugin) selectedProfileIndex + 1 else selectedProfileIndex
                        configurationListView.scrollTo(scrollIndex, true)
        ...
        private fun hasKwnetPlugin(): Boolean {
            return try {
                requireContext().packageManager.getInstalledApplications(PackageManager.GET_META_DATA)
                    .any { it.packageName.contains("kwnet", ignoreCase = true) }
            } catch (_: Exception) {
                false
            }
        }

        private fun findFirstKwnetPluginPackage(): String? {
            return try {
                requireContext().packageManager.getInstalledApplications(PackageManager.GET_META_DATA)
                    .firstOrNull { it.packageName.contains("kwnet", ignoreCase = true) }
                    ?.packageName
            } catch (_: Exception) {
                null
            }
        ...
        inner class VirtualPluginHolder(view: View) : RecyclerView.ViewHolder(view) {
            private val editButton: ImageView = view.findViewById(R.id.edit_plugin)

            fun bind() {
                itemView.setOnClickListener {
                    try {
                        val intent = Intent(itemView.context, AppManagerActivity::class.java).apply {
                ...
                editButton.setOnClickListener {
                    val targetPkg = findFirstKwnetPluginPackage() ?: return@setOnClickListener
                    try {
        ...
        inner class ConfigurationHolder(val view: View) : RecyclerView.ViewHolder(view),
            ...
            fun bind(proxyEntity: ProxyEntity, trafficData: TrafficData? = null) {
                ...
                val isFloatingProfile = proxyEntity.displayName() == "悬浮窗"
                        ...
                        runOnDefaultDispatcher {
                            ...
                            if (update) {
                                ...
                                if (isFloatingProfile) {
                                    if (DataStore.serviceState.started && reloadAccess.tryLock()) {
                                        SagerNet.stopService()
                                        onMainDispatcher {
                                            FloatingWindowService.start(view.context.applicationContext)
                                        }
                                        reloadAccess.unlock()
                                    }
                                } else if (DataStore.serviceState.canStop && reloadAccess.tryLock()) {
                ...
                editButton.setOnClickListener {
                    if (KIOSK_MODE && !select && isFloatingProfile) {
                        it.context.startActivity(
                            Intent(it.context, FloatingWindowSettingsActivity::class.java)
                        )
                    } else {
                        it.context.startActivity(
                ...
                if (KIOSK_MODE && !select) {
                    shareLayout.isGone = true
                    removeButton.isGone = true
                    profileStatus.setOnClickListener(null)

                    editButton.isGone = !isFloatingProfile
