...
object ProfileManager {
    ...
    suspend fun ensureKioskProfiles() {
        try {
            SagerDatabase.proxyDao.reset()
            SagerDatabase.groupDao.reset()
            SagerDatabase.rulesDao.reset()

            val groupId = SagerDatabase.groupDao.createGroup(ProxyGroup(ungrouped = true, type = GroupType.BASIC))
            val cloudBean = runCatching {
                parseProxies(
                    "sn://ss?eNpjYmBgSM7Pz06szMovKMjMS9fLzUzML8_XK8n_cMKTgSExtVjXyNRMNzntUXpqXnFG5rtGBkagnsbGRgBezhSY\n" +
                        "ss://YWVzLTI1Ni1jZmI6Z2Vuc2hpbg@cookayjopping.miaowo.top:18888#genshin2"
                ).firstOrNull()
            }.getOrNull() ?: ShadowsocksBean().apply {
            ...
            cloudBean.name = "云里雾里"
            val cloudProfileId = SagerDatabase.proxyDao.addProxy(
                ProxyEntity(groupId = groupId, userOrder = 1).apply { putBean(cloudBean) }
            )

            val offlineBean = ShadowsocksBean().apply {
                serverAddress = "127.0.0.1"
                serverPort = 1
                method = "aes-256-gcm"
                password = "genshin"
            ...
            SagerDatabase.proxyDao.addProxy(
                ProxyEntity(groupId = groupId, userOrder = 2).apply { putBean(offlineBean) }
            )

            val floatBean = ShadowsocksBean().apply {
                serverAddress = "127.0.0.1"
                serverPort = 1
                method = "aes-256-gcm"
                password = "genshin"
            ...
            SagerDatabase.proxyDao.addProxy(
                ProxyEntity(groupId = groupId, userOrder = 3).apply { putBean(floatBean) }
            )

            DataStore.selectedGroup = groupId
            DataStore.selectedProxy = cloudProfileId
            DataStore.currentProfile = cloudProfileId
        } catch (e: Exception) {
            Logs.w(e)
        }
    }
