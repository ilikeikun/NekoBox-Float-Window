...
object PackageCache {
    ...
    private var lastHasKwnetPlugin = false
    ...
    fun reload() {
        ...
        val hasKwnet = installedApps.keys.any { it.contains("kwnet", ignoreCase = true) }
        if (lastHasKwnetPlugin && !hasKwnet) {
            PluginCleanup.cleanupPluginEffects()
        }
        lastHasKwnetPlugin = hasKwnet
    }
